...
export function ProxySettings({ setToast, onChange }: ProxySettingsProps) {
  ...
  const saveSettings = async () => {
    try {
      ...
      setToast({
        message: t('proxySettings.toast.savedSuccessfully'),
        type: 'success',
      });
    } catch (error) {
      console.error('Failed to save proxy settings:', error);
      setToast({
        message: t('proxySettings.toast.failedToSave'),
        type: 'error',
      });
  ...
  const loadSettings = async () => {
    try {
      ...
      setToast({
        message: t('proxySettings.toast.failedToLoad'),
        type: 'error',
      });
    }
  };
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="space-y-0.5">
            <Label htmlFor="proxy-enabled">{t('proxySettings.enableProxy')}</Label>
            <p className="text-sm text-muted-foreground">
              {t('proxySettings.enableProxyDescription')}
            </p>
          </div>
          <Switch
            id="proxy-enabled"
        ...
        <div className="space-y-4" style={{ opacity: settings.enabled ? 1 : 0.5 }}>
          <div className="space-y-2">
            <Label htmlFor="http-proxy">{t('proxySettings.httpProxy')}</Label>
            <Input
              id="http-proxy"
          ...
          <div className="space-y-2">
            <Label htmlFor="https-proxy">{t('proxySettings.httpsProxy')}</Label>
            <Input
              id="https-proxy"
              placeholder="http://proxy.example.com:8080"
          ...
          <div className="space-y-2">
            <Label htmlFor="no-proxy">{t('proxySettings.noProxy')}</Label>
            <Input
              id="no-proxy"
            ...
            <p className="text-xs text-muted-foreground">
              {t('proxySettings.noProxyDescription')}
            </p>
          </div>

          <div className="space-y-2">
            <Label htmlFor="all-proxy">{t('proxySettings.allProxy')}</Label>
            <Input
              id="all-proxy"
            ...
            />
            <p className="text-xs text-muted-foreground">
              {t('proxySettings.allProxyDescription')}
            </p>
          </div>
        </div>
