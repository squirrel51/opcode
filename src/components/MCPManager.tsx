...
export const MCPManager: React.FC<MCPManagerProps> = ({
  ...
  const loadServers = async () => {
    try {
      ...
      setError(t("mcpManager.loadError"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleServerRemoved = (name: string) => {
    ...
    setToast({ message: t("mcpManager.serverRemovedSuccess", { name }), type: "success" });
  };
  ...
  const handleImportCompleted = (imported: number, failed: number) => {
    ...
    if (failed === 0) {
      setToast({ 
        message: t("mcpManager.importSuccess", { count: imported }), 
        type: "success" 
      });
    } else {
      setToast({ 
        message: t("mcpManager.importPartialSuccess", { imported, failed }), 
        type: "error" 
      });
    }
  };
      ...
      <div className="max-w-6xl mx-auto flex flex-col h-full">
        ...
        <div className="p-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-heading-1">{t("mcpManager.title")}</h1>
              <p className="mt-1 text-body-small text-muted-foreground">
                {t("mcpManager.subtitle")}
              </p>
            </div>
          </div>
        </div>
