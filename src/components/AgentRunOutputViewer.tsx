...
export function AgentRunOutputViewer({ 
  ...
  useEffect(() => {
    const loadAgentRun = async () => {
      try {
        ...
        setRun(agentRun);
        updateTabTitle(tabId, `${t('agentRunOutputViewer.agent')}: ${agentRun.agent_name || t('agentRunOutputViewer.unknown')}`);
  ...
  }, [agentRunId, tabId, updateTabTitle, updateTabStatus, t]);
  ...
  const loadOutput = async (skipCache = false) => {
    ...
    try {
      ...
      setToast({ message: t('agentRunOutputViewer.loadOutputError'), type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const setupLiveEventListeners = async () => {
    ...
    try {
      ...
      const completeUnlisten = await listen<boolean>(`agent-complete:${run!.id}`, () => {
        setToast({ message: t('agentRunOutputViewer.executionCompleted'), type: 'success' });
      ...
      const cancelUnlisten = await listen<boolean>(`agent-cancelled:${run!.id}`, () => {
        setToast({ message: t('agentRunOutputViewer.executionCancelled'), type: 'error' });
      });
  ...
  const handleCopyAsJsonl = async () => {
    ...
    setToast({ message: t('agentRunOutputViewer.copiedAsJsonl'), type: 'success' });
  };

  const handleCopyAsMarkdown = async () => {
    if (!run) return;
    let markdown = `# ${t('agentRunOutputViewer.agentExecution')}: ${run.agent_name}\n\n`;
    markdown += `**${t('agentRunOutputViewer.task')}:** ${run.task}\n`;
    markdown += `**${t('agentRunOutputViewer.model')}:** ${run.model === 'opus' ? t('agentRunOutputViewer.claudeOpus') : t('agentRunOutputViewer.claudeSonnet')}\n`;
    markdown += `**${t('agentRunOutputViewer.date')}:** ${formatISOTimestamp(run.created_at)}\n`;
    if (run.metrics?.duration_ms) markdown += `**${t('agentRunOutputViewer.duration')}:** ${(run.metrics.duration_ms / 1000).toFixed(2)}s\n`;
    if (run.metrics?.total_tokens) markdown += `**${t('agentRunOutputViewer.totalTokens')}:** ${run.metrics.total_tokens}\n`;
    if (run.metrics?.cost_usd) markdown += `**${t('agentRunOutputViewer.cost')}:** $${run.metrics.cost_usd.toFixed(4)} USD\n`;
    markdown += `\n---\n\n`;

    for (const msg of messages) {
      if (msg.type === "system" && msg.subtype === "init") {
        markdown += `## ${t('agentRunOutputViewer.systemInitialization')}\n\n`;
        markdown += `- ${t('agentRunOutputViewer.sessionId')}: \`${msg.session_id || t('agentRunOutputViewer.notAvailable')}\`\n`;
        markdown += `- ${t('agentRunOutputViewer.model')}: \`${msg.model || 'default'}\`\n`;
        if (msg.cwd) markdown += `- ${t('agentRunOutputViewer.workingDirectory')}: \`${msg.cwd}\`\n`;
        if (msg.tools?.length) markdown += `- ${t('agentRunOutputViewer.tools')}: ${msg.tools.join(', ')}\n`;
        markdown += `\n`;
      } else if (msg.type === "assistant" && msg.message) {
        markdown += `## ${t('agentRunOutputViewer.assistant')}\n\n`;
        for (const content of msg.message.content || []) {
          if (content.type === "text") {
            markdown += `${content.text}\n\n`;
          } else if (content.type === "tool_use") {
            markdown += `### ${t('agentRunOutputViewer.tool')}: ${content.name}\n\n`;
            markdown += `\`\`\`json\n${JSON.stringify(content.input, null, 2)}\n\`\`\`\n\n`;
          }
        }
        if (msg.message.usage) {
          markdown += `*${t('agentRunOutputViewer.tokens')}: ${msg.message.usage.input_tokens} ${t('agentRunOutputViewer.in')}, ${msg.message.usage.output_tokens} ${t('agentRunOutputViewer.out')}*\n\n`;
        }
      } else if (msg.type === "user" && msg.message) {
        markdown += `## ${t('agentRunOutputViewer.user')}\n\n`;
        for (const content of msg.message.content || []) {
          if (content.type === "text") {
            markdown += `${content.text}\n\n`;
          } else if (content.type === "tool_result") {
            markdown += `### ${t('agentRunOutputViewer.toolResult')}\n\n`;
            markdown += `\`\`\`\n${content.content}\n\`\`\`\n\n`;
          }
        }
      } else if (msg.type === "result") {
        markdown += `## ${t('agentRunOutputViewer.executionResult')}\n\n`;
        if (msg.result) {
          markdown += `${msg.result}\n\n`;
        }
        if (msg.error) {
          markdown += `**${t('agentRunOutputViewer.error')}:** ${msg.error}\n\n`;
        }
      }
    }
    ...
    setCopyPopoverOpen(false);
    setToast({ message: t('agentRunOutputViewer.copiedAsMarkdown'), type: 'success' });
  };
  ...
  const handleStop = async () => {
    ...
    try {
      ...
      if (success) {
        ...
        setToast({ message: t('agentRunOutputViewer.executionStopped'), type: 'success' });
        ...
        const stopMessage: ClaudeStreamMessage = {
          ...
          subtype: "error",
          is_error: true,
          result: t('agentRunOutputViewer.executionStoppedByUser'),
          duration_ms: 0,
          usage: {
            input_tokens: 0,
            output_tokens: 0
        ...
        setToast({ message: t('agentRunOutputViewer.stopFailed'), type: 'error' });
      }
    } catch (err) {
      console.error('[AgentRunOutputViewer] Failed to stop agent:', err);
      setToast({ 
        message: `${t('agentRunOutputViewer.stopExecutionFailed')}: ${err instanceof Error ? err.message : t('agentRunOutputViewer.unknownError')}`, 
        type: 'error' 
      });
    }
  };
  ...
  const formatDuration = (ms?: number) => {
    if (!ms) return t('agentRunOutputViewer.notAvailable');
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">{t('agentRunOutputViewer.loadingAgentRun')}</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <CardTitle className="text-lg flex items-center gap-2">
                    ...
                    {run.status === 'running' && (
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-xs text-green-600 font-medium">{t('agentRunOutputViewer.running')}</span>
                      </div>
                    )}
                  </CardTitle>
                  ...
                  <div className="flex items-center gap-3 text-xs text-muted-foreground mt-2">
                    <Badge variant="outline" className="text-xs">
                      {run.model === 'opus' ? t('agentRunOutputViewer.claudeOpus') : t('agentRunOutputViewer.claudeSonnet')}
                    </Badge>
                    <div className="flex items-center gap-1">
              ...
              <div className="flex items-center gap-1">
                <Popover
                  ...
                  content={
                    <div className="w-44 p-1">
                      ...
                      >
                        {t('agentRunOutputViewer.copyAsJsonl')}
                      </Button>
                      <Button
                        variant="ghost"
                        size="sm"
                      ...
                      >
                        {t('agentRunOutputViewer.copyAsMarkdown')}
                      </Button>
                    </div>
                  }
                ...
                <Button
                  ...
                  title={isFullscreen ? t('agentRunOutputViewer.exitFullscreen') : t('agentRunOutputViewer.enterFullscreen')}
                  className="h-8 px-2"
                >
                  {isFullscreen ? (
                ...
                <Button
                  ...
                  disabled={refreshing}
                  title={t('agentRunOutputViewer.refreshOutput')}
                  className="h-8 px-2"
                >
                ...
                {run.status === 'running' && (
                  <Button
                    ...
                    onClick={handleStop}
                    disabled={refreshing}
                    title={t('agentRunOutputViewer.stopExecution')}
          ...
          {loading ? (
              <div className="flex items-center justify-center h-full">
                <div className="flex items-center space-x-2">
                  ...
                  <span>{t('agentRunOutputViewer.loadingOutput')}</span>
                </div>
              </div>
            ) : messages.length === 0 ? (
              <div className="flex items-center justify-center h-full text-muted-foreground">
                <p>{t('agentRunOutputViewer.noOutputAvailable')}</p>
              </div>
            ) : (
              <div 
                ref={scrollAreaRef}
          ...
          <div className="flex items-center justify-between p-4 border-b">
            ...
            <div className="flex items-center gap-2">
              <Popover
                trigger={
                  ...
                  >
                    ...
                    {t('agentRunOutputViewer.copyOutput')}
              ...
              >
                ...
                {t('agentRunOutputViewer.exitFullscreen')}
              </Button>
            </div>
          </div>
          <div 
            ...
            <div className="max-w-4xl mx-auto space-y-2">
              {messages.length === 0 ? (
                <div className="text-center text-muted-foreground py-8">
                  {t('agentRunOutputViewer.noOutputAvailable')}
                </div>
              ) : (
                <>
                  <AnimatePresence>
